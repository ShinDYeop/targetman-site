...
export function PhotoFrame({
  ...
  alt: string;
  empty: string;
  /** 상세 페이지용. 사진을 자르지 않고 전부 보여 주고, 누르면 크게 열립니다. */
  zoom?: boolean;
}) {
  ...
  function moveLb(n: number) {
    setLb(Math.max(0, Math.min(keys.length - 1, n)));
  }
  ...
  return (
    <>
      <div className="t-ph" data-detail={zoom ? "1" : undefined}>
